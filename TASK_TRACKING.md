...
**Photasa 黄金规则：** [ROADMAP.md](./ROADMAP.md) → Golden rule — Rust 重写后端；Electron/TS 仅作行为规格，禁止复制 TS 到 Tauri。
...
| 应用偏好（文昌）落盘 | [0107](./.spec/rfc/0107-tauri-wenchang-preferences-storage.md)                 | 🔴 High   | 🔨 In Progress（生产打包缺失，见 RFC） | **是**（生产环境 workflow 全部加载失败） |
...
- [x] 新增 workspace crate：`crates/photasa-preference`（2026-07-19 由 `wenchang-preferences` 改名，Tauri service 名 `wenchang` 不变）
...
- [x] 验证证据（历史，crate 改名前记录）：`cargo test -p wenchang-preferences` + `cargo build -p photasa`（2026-04-12）；改名后 2026-07-19 重跑 `cargo test -p photasa-preference` → 5 passed
...
| 扫描增量缓存                | [0105](./.spec/rfc/completed/0105-tauri-scan-incremental-cache.md)     | 🔴 High   | ✅ Done | `scan_cache` + `scan_runner`；progress total 对齐 Electron          |
...
**父跟踪 RFC：[0097](./.spec/rfc/completed/0097-tauri-legacy-api-deferred-surface.md)**。全部交付物为 Rust；Electron/TS 仅作契约对照。
...
| `.photasa.json` 缩略图路径 parity | [0116](./.spec/rfc/completed/0116-tauri-photasa-config-thumbnail-parity.md)    | 🔴 High   | ✅ Done | Electron `toRelativeThumbnailPath`；fix legacy stubs + folder race             |
| 扫描流水线 Electron 契约          | [0117](./.spec/rfc/completed/0117-tauri-scan-pipeline-parity.md)               | 🔴 High   | ✅ Done | SKIP-only 递归 + SKIP progress `(N,N)` 已修；52 scan 测试通过                  |
...
| ------------------------------------------------------------------------------ | -------------------------------------------------------- | -------------- | ----------------------------------------------------------------------------------- |
| [0097](./.spec/rfc/completed/0097-tauri-legacy-api-deferred-surface.md)        | legacy-api 与 Electron 1:1 跟踪（**父 RFC**）            | ✅ Implemented | Phase 7（0111–0114）全部完成                                                        |
...
| [0073](./.spec/rfc/completed/0073-tauri-ui-migration-adapter.md)               | UI 迁移与适配层设计                                  | Implemented                              | AI     | v2.1.0         |
...
| [0097](./.spec/rfc/completed/0097-tauri-legacy-api-deferred-surface.md)        | legacy-api 与 Electron 1:1 跟踪                      | ✅ Implemented（Photasa Active）         | AI     | v2.1.0         |
