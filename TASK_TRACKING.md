...
| ImageList TanStack 虚拟网格    | [0161](./.spec/rfc/completed/0161-imagelist-tanstack-virtual-grid.md)              | 🟡 Medium | ✅ Done — `ImageList` 改用 `VirtualizedGrid`；删 inline virtualizer；Vitest 通过                    | 否                                  |
| `design-thumbnail` crate       | [0004](./.spec/rfc/completed/0004-local-design-file-preview-rust.md)               | 🟡 Medium | ✅ Done — 通用 Rust crate；PSD / AI 真实样本及三平台 check 通过；Photasa 集成后续                   | 否                                  |
| PSD / AI 缩略图库与主流程集成  | [0173](./.spec/rfc/0173-photasa-design-thumbnail-integration.md)                   | 🟡 Medium | 🔎 Under Review — 通用库已完成；Photasa 集成设计已完成复审修订                                      | 否                                  |
...
| 菜单重设计（File/Edit/Window） | [0169](./.spec/rfc/completed/0169-menu-redesign-common-and-photasa-domain.md)      | 🟡 Medium | ✅ Closed — 增量 `update_menu_item` + 领域菜单 + 编辑/最小化 i18n（2026-07-26）                     | 否                                  |
| macOS Help 子菜单空白          | [0170](./.spec/rfc/completed/0170-macos-help-menu-empty-upstream-bug.md)           | 🔴 High   | ✅ Closed — `muda 0.17.2` + 启动 IPC 时序（2026-07-26）                                             | 否                                  |
| macOS 原生 Help 菜单           | [0171](./.spec/rfc/completed/0171-macos-native-help-menu.md)                       | 🟡 Medium | ✅ Closed — Report Issue + Explore + Getting Started + i18n（2026-07-26）                           | 否                                  |
...
| ------------------------------------------------------------------------------ | -------------------------------------------------------- | --------------------------------- | ----------------------------------------------------------------------------------- |
| [0173](./.spec/rfc/0173-photasa-design-thumbnail-integration.md)               | PSD / AI 缩略图库与 Photasa 主流程集成                   | 🔎 Under Review                   | 通用库已完成；待交付分类、decoder 路由、temp album integration、Tauri smoke         |
...
| [0161](./.spec/rfc/completed/0161-imagelist-tanstack-virtual-grid.md)          | ImageList TanStack 虚拟网格整合               | ✅ Implemented | `ImageList` → `VirtualizedGrid`；换文件夹滚顶；`scrollToImageIndex`；加载 overlay 不卸载 grid                       |
| [0004](./.spec/rfc/completed/0004-local-design-file-preview-rust.md)           | `design-thumbnail` 通用 Rust crate            | ✅ Implemented | 独立完成 PSD / PDF-compatible AI → 缩略图文件；Photasa 集成不在本阶段                                               |
...
| [0163](./.spec/rfc/completed/0163-tauri-posthog-telemetry.md)                  | PostHog 遥测 + 显式同意机制                   | ✅ Implemented | `posthog-rs`/`posthog-js`、三态 consent、首次对话框、Settings 撤回；Vitest + Rust tests                             |
| [0164](./.spec/rfc/completed/0164-cleanup-legacy-node-packages.md)             | 物理清理 14 个废弃 Node 包                    | ✅ Implemented | 删 config-core/import/maliang\*/scan 等 14 包；保留 `common` + `base-tree`；release-please 14 extra-files           |
| [0165](./.spec/rfc/completed/0165-base-tree-package-and-folder-restore.md)     | `@photasa/base-tree` + 启动树 + 虚拟化        | ✅ Implemented | `@photasa/base-tree` 包；启动展开+选中；**2026-07-25**：FolderList `virtual=false`、展开不跳顶（RFC Amendment）     |
...
| [0008](./docs/rfc/0008-scan-strategy-optimization.md) … [0061](./docs/rfc/0061-zouwu-workflow-visualization.md) | v2.0 legacy 能力草案         | Draft / 🔨  | Legacy contract reference；RFC 0004 已独立实现并归档 |
...
| ------------------------------------------------------------------------------ | ---------------------------------------------------- | ----------------------------------------- | ----------------------------------------------------------- | -------------- |
| [0004](./.spec/rfc/completed/0004-local-design-file-preview-rust.md)           | `design-thumbnail` PSD / AI crate                    | ✅ Implemented（crate only）              | 通用 crate；真实样本与三平台 check 已通过；Photasa 后续依赖 |
| [0173](./.spec/rfc/0173-photasa-design-thumbnail-integration.md)               | PSD / AI 缩略图库与 Photasa 主流程集成               | 🔎 Under Review                           | Phase A 通用库完成；Phase B Photasa 主流程待批准            |
