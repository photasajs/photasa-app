...
RFC 权威位置：`.spec/rfc/`。索引与 roadmap：`.spec/ROADMAP.md`。

**手动流程**（无 CLI 工具）：

1. 在 `.spec/rfc/` 新建 `NNNN-short-slug.md`（沿用模板）
2. 同步更新 `.spec/ROADMAP.md`（Active RFCs 表）
3. 完成 → 移入 `.spec/rfc/completed/`，更新 Implemented 表
4. 拒绝 → 移入 `.spec/rfc/rejected/`，更新 Rejected 表并注明原因
...
**适用范围**：`apps/desktop/src/main/`、`packages/@photasa/` 下 Main 进程相关文件
...
**适用范围**：`apps/desktop/src/renderer/` 下的所有文件
...
- **绝对禁止**在 Main 进程（`apps/desktop/src/main/`、`packages/@photasa/`）中使用人界风格词汇
- **绝对禁止**在 Renderer 进程（`apps/desktop/src/renderer/`）中使用天界风格词汇
...
**文档参考** - 详见 `.spec/rfc/completed/0031-maliang-image-processing-engine.md`
...
✅ 步骤1: 接口层定义
   └─ apps/desktop/src/renderer/src/interfaces/fang-xuan-ling.interface.ts
      └─ ZOUZHE_MATTERS 中添加常量

✅ 步骤2: 袁天罡 intentMapping 映射 ⚠️ 容易遗漏！
   └─ apps/desktop/src/renderer/src/services/yuantiangang/yuantiangang.ts
      └─ convertFuluToUICommand() 中的 intentMapping 添加映射
      └─ 格式: [ZOUZHE_MATTERS.XXX]: "workflow_name"

✅ 步骤3: 房玄龄处理逻辑（如需要）
   └─ apps/desktop/src/renderer/src/services/fangxuanling/accessors/<accessor>.ts
      └─ 添加业务逻辑处理

✅ 步骤4: matter-sync.yml 配置（如需自动同步Store）
   └─ apps/desktop/src/renderer/src/services/fangxuanling/store-automation/matter-sync.yml
      └─ 添加 autoSync 配置
...
✅ 步骤5: UserIntent 类型定义 ⚠️ 容易遗漏！
   └─ packages/@photasa/tianshu/src/types/commands.ts
      └─ UserIntent 联合类型中添加 "workflow_name"

✅ 步骤6: intentToWorkflowMap 映射 ⚠️ 容易遗漏！
   └─ packages/@photasa/tianshu/src/core/TianshuEngine.ts
      └─ intentToWorkflowMap 中添加映射
      └─ 格式: workflow_name: "domain/workflow_name"

✅ 步骤7: 天界工作流文件
   └─ apps/desktop/src/main/engines/tianshu/workflows/<domain>/<workflow_name>.yml
      └─ 创建对应的工作流文件
```
...
grep -o "ZOUZHE_MATTERS\.[A-Z_]*" apps/desktop/src/renderer/src/interfaces/fang-xuan-ling.interface.ts | sort -u > /tmp/matters.txt
grep -o "\[ZOUZHE_MATTERS\.[A-Z_]*\]" apps/desktop/src/renderer/src/services/yuantiangang/yuantiangang.ts | sort -u > /tmp/mappings.txt
diff /tmp/matters.txt /tmp/mappings.txt

# 2. 检查天界 UserIntent 类型是否完整
grep -o '"[a-z_]*"' packages/@photasa/tianshu/src/types/commands.ts | sort -u

# 3. 检查天界 intentToWorkflowMap 是否完整
grep -o '[a-z_]*:' packages/@photasa/tianshu/src/core/TianshuEngine.ts | head -20
```
