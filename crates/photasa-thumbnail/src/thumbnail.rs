 ...
 * 缩略图命令
 *
 * 实现 create_thumbnail / remove_thumbnail，对应 contract reference MaLiang 引擎的核心能力：
...
use crate::placeholder::{extension_label_from_path, render_labeled_placeholder};
...
impl ThumbnailResponse {
 ...
 pub fn ok_fallback(file: String) -> Self {
 Self {
 success: true,
 file: Some(file),
 error: None,
...
pub async fn create_thumbnail(request: ThumbnailRequest) -> ThumbnailResponse {
 tokio::task::spawn_blocking(move || decode_thumbnail_blocking(&request))
 .await
 .unwrap_or_else(|e| ThumbnailResponse::err(format!("线程异常: {e}")))
}

fn decode_thumbnail_blocking(request: &ThumbnailRequest) -> ThumbnailResponse {
...
fn make_video_thumbnail_blocking(
 ...
 use crate::video::save_video_thumbnail;
...
pub async fn remove_thumbnail(path: PathBuf) -> Result<(), String> {
 tokio::task::spawn_blocking(move || remove_thumbnail_blocking(&path))
 .await
 .map_err(|e| format!("线程异常: {e}"))?
}

fn remove_thumbnail_blocking(path: &Path) -> Result<(), String> {
 if !path.exists() {
 return Ok(());
 }
...
mod raw_placeholder_tests {
 ...
 let dir = std::env::temp_dir().join(format!("photasa-raw-thumb-{}", std::process::id()));
 ...
 let res = decode_thumbnail_blocking(&req);
