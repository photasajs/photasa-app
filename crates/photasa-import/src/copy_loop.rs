...
pub fn run_import_file_loop(
    ...
    let target_pb = target_path.to_path_buf();
    ...
    for src_s in files {
        if !wait_unpaused_or_cancel(cancel, paused) {
        ...
        let date_sub = date_subpath_for_import_source(src, meta);
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let d = std::env::temp_dir().join(format!("photasa-imp-exec-{label}-{}", uuid::Uuid::new_v4()));
    ...
    fn take_duplicate_strategy_variants() {
        assert_eq!(take_duplicate_strategy(&json!({"duplicateStrategy": "skip"})), "skip");
        assert_eq!(
            take_duplicate_strategy(&json!({"duplicateStrategy": "overwrite"})),
            "overwrite"
        );
        assert_eq!(take_duplicate_strategy(&json!({"duplicateStrategy": "rename"})), "rename");
    ...
    fn collect_files_sorts_dedups_and_filters() {
        ...
        assert_eq!(collect_files(&cfg), vec!["/a.jpg".to_string(), "/b.jpg".to_string()]);
    ...
    fn copy_one_rename_increments_when_alt_exists() {
        ...
        let (ok, p) = copy_one(&src, &dest_dir, "rename").unwrap();
        assert!(ok);
        assert_eq!(p, dest_dir.join("x_2.png"));
        let _ = fs::remove_dir_all(&root);
    }
    ...
    fn run_import_file_loop_counts_copy_errors() {
        ...
        .unwrap();
        match end {
            ImportLoopEnd::Completed { errors, successful, .. } => {
                assert_eq!(errors, 1);
                assert_eq!(successful, 0);
            }
            _ => panic!("expected completed"),
