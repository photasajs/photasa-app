//! `.photasa.json` 读写 — 与 legacy-api / `@photasa/config-core` 契约对齐
//!
...
/// 缩略图相对路径：`.photasaoriginals/thumbnail-{fileName}.png`（contract reference `toRelativeThumbnailPath`）
...
/// contract reference `shortenThumbnailName`: `.photasaoriginals/` + basename
...
/// 读取 `.photasa.json`（contract reference `readConfig`：不静默改写磁盘）
...
/// contract reference `removeFromPhotoList`
...
mod tests {
 ...
 #[test]
    fn parse_photo_list_accepts_reference_objects() {
 let raw = json!([
 {
 "path": "a.jpg",
 ...
 #[test]
    fn shorten_thumbnail_relative_path_matches_reference() {
 assert_eq!(
 ...
 #[test]
    fn fix_config_sync_matches_reference_shorten_only() {
 ...
 #[test]
    fn thumbnail_path_matches_reference_contract() {
 assert_eq!(
