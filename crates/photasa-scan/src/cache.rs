//! `.photasa-folder.json` 增量扫描缓存（RFC 0105 — Rust 重写，磁盘格式与 legacy-api 契约兼容）
...
/// 与 legacy-api `IncrementalCache` / `FolderCache` 兼容的 JSON 结构
...
impl FolderScanCache {
 ...
 /// `(processed, total)` — 与 legacy-api `mergeDirectoryScanProgressWithCache` 一致
 ...
 /// 将已处理文件从 pending 移至 processed（processed 存 basename，与 legacy-api 一致）
...
/// contract reference `IncrementalCacheManager` — 批量写入 `.photasa-folder.json`
