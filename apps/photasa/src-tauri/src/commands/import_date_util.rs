/*!
 * 导入日期子路径（RFC 0104）：与 `import_preview` / Electron `generateDatePath` 共用同一套 UTC 日期解析与 `{year}/{YYYYMMDD}` 格式。
 */
...
pub(crate) fn generate_date_path_utc(dt: DateTime<Utc>) -> String {
    let d = dt.date_naive();
    let y = d.year();
    let m = d.month();
    let day = d.day();
...
pub(crate) fn compute_fallback_date_utc(created_rfc: &str, modified_rfc: &str) -> DateTime<Utc> {
...
pub(crate) fn determine_group_target_utc(main: &Value) -> DateTime<Utc> {
...
pub(crate) fn rfc3339_pair_from_fs_meta(meta: &std::fs::Metadata) -> (String, String) {
    let modified = meta
        .modified()
        .ok()
...
/// 将 `extract_metadata` 结果并入 `FileInfo` 形状（对齐 `processFileGroup`）
pub(crate) fn merge_extract_into_file_info(
    fi: &mut Value,
    meta: &Value,
    created_rfc: &str,
    modified_rfc: &str,
...
/// 单文件：`extract_metadata` + 与预览相同的日期链 → `YYYY/YYYYMMDD` 子路径
pub(crate) fn date_subpath_for_import_source(src: &Path) -> String {
    let path_str = src.to_string_lossy().replace('\\', "/");
    let Ok(meta) = std::fs::metadata(src) else {
        return generate_date_path_utc(Utc::now());
    };
    let (created_rfc, modified_rfc) = rfc3339_pair_from_fs_meta(&meta);
    let (is_image, is_video) = classify_media(src).unwrap_or((false, false));
    ...
    let extracted = extract_metadata_request(&extract_req).unwrap_or_else(|_| json!({}));
    let mut fi = json!({
        "path": path_str,
...
pub(crate) fn join_date_subpath(root: &Path, date_subpath: &str) -> PathBuf {
...
pub(crate) fn relative_target_path_for_import(date_subpath: &str, dest_file_name: &str) -> String {
...
mod tests {
    ...
    fn date_subpath_follows_file_mtime_when_no_exif() {
        ...
        let sub = date_subpath_for_import_source(&path);
