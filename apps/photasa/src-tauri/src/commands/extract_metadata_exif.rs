...
/// RFC 0093：与 legacy-api `preload/exif-helper.resolveExifDate` + `moment(..., "YYYY/YYYYMMDD")` 一致。
/// 仅**图片**写入 `年/年日月日` 子目录（`historical preload/__tests__/exif-helper.spec.ts` 的 `^\d{4}/\d{8}$`）；**视频**返回 `None`，文件落在目标根目录（与 preload 中 `checkExifDate` 拒识视频后 `targetName` 为空一致）。
