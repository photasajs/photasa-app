//! 配置引擎健康状态桥 — 对齐 legacy-api `ConfigService.handleEngineStatus` → `picasa:engine-status`。
//!
...
/// 与 legacy-api `engine: "sibu"` 一致（四部典籍 / config worker 预留名）。
pub const ENGINE_SIBU: &str = "sibu";

/// contract reference `ConfigResponse` 中 `action: "engine-status"` 的 Rust 子集。
...
/// 与 legacy-api `webContents.send("picasa:engine-status", { ... })` 同形。
#[derive(Debug, Clone, Serialize)]
...
/// 构建与 legacy-api 对齐的 `picasa:engine-status` 载荷。
...
mod tests {
 ...
 #[test]
 fn build_event_matches_reference_shape() {
