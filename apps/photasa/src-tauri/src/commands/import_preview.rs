/*!
 * RFC 0070：导入预览 — 与 legacy-api `import-worker` 预览链 **1:1**：
...
fn estimate_import_duration_reference_style(total_files: usize, total_size: u64) -> f64 {
...
fn scan_preview_blocking(
 ...
 let estimated = estimate_import_duration_reference_style(files_found, total_size);
...
mod target_structure_tests {
 ...
 let e = estimate_import_duration_reference_style(10, 100 * 1024 * 1024);
