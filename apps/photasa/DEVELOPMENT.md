...
macOS Dock：`tauri dev` 裸二进制默认显示 Cargo 包名 `photasa`（不是窗口标题）。Dev 通道通过 `mainBinaryName: "Photasa Dev"` 让 CLI 重命名可执行文件，并在 `main()` 最早阶段调用 `NSProcessInfo.setProcessName`。改完后须**完全退出**旧进程再 `pnpm dev`；若 Dock 仍显示 `photasa`，从 Dock 移除旧图标后重开。

**Dev 图标**：蓝色底 + 右下角 `DEV` 角标（`src-tauri/icons/icon.dev.svg` → `icons-dev/`）。改 SVG 后运行 `pnpm --filter @photasa/photasa run generate:dev-icons` 并提交产物。
