...
- **Status**: ✅ Photasa 已实现核心：`import_photos_legacy`（`commands/import_legacy.rs`）+ 事件 `picasa:import-photos-legacy`；前端 `legacy-api.importPhotos` `invoke` 与监听。**测试：** Rust `copy_with_unique_name` 单测（`import_legacy.rs`）；前端 `legacy-api-import-photos.test.ts`（invoke 失败、`next`/`complete`、`created`→`Date`）。**余量：** 与 legacy-api 大规模 fixtures 的端到端对拍（可选）。
- **RFC PR**: (leave empty)
...
## Callback / event shape（与 legacy-api `photo-import.ts` + `FileAction` 对齐）

- **事件名**：`picasa:import-photos-legacy`（Tauri）；contract reference 为 preload 进程内 RxJS，无同名 IPC 事件。
...
- 可选：用固定媒体夹具跑 legacy vs Tauri 双端回归，自动 diff 回调序列。
