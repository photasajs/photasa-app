    ...
    try {
        ...
        if (shouldUseMaLiang(arg.path)) {
            try {
                ...
                logger.warn(
                    `[thumbnail-handler] Ma-Liang failed, fallback to legacy: ${maLiangError.message}`,
                );
            }
        }
        ...
        logger.error(
            "[thumbnail-handler] Failed to create thumbnail: " + arg.path + " due to: " + error,
        );
        return arg;
    }
}
...
class HeicBrush {
    public async createMiniature(
        ...
        const results = await Promise.all([
            ...
            options.generatePreview ? this.generatePreview(rgbaBuffer, options.previewPath) : null,
        ]);
