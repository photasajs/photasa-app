...
- **作者**: AI Assistant
- **状态**: ✅ 已完成（伞形索引 RFC，2026-07-21 复核归档；子 RFC 全部已交付，正文内技术方案/checklist/依赖版本示例已过时，仅存档参考，不要按内容实施——以 ROADMAP.md「RFC 仓库索引」+ 各子 RFC 现状为准）
- **创建日期**: 2025-01-02
- **关联项目**: Photasa Desktop (contract reference)
- **关联 RFC**:
...
本 RFC 是总体架构/索引文档（创建于 2025-01-02），不是独立交付物。子 RFC（0068-0072）均已交付并归档完成（0068 扫描、0069 缩略图、0070 导入、0071 配置、0072 天枢），Rust 后端替代原设想的"WASM 过渡"直接完成（zouwu-core 是纯 Rust 重写，未经过 WASM 阶段，正文"工作流引擎迁移"章节的选项 B 描述已不适用）。正文其余部分（依赖版本 `tauri = "1.5"`、checklist 未勾选项、时间估算等）是 2025 年初的规划快照，不代表当前实现状态——查具体实现现状请看各子 RFC 或 ROADMAP.md。
...
在 `apps` 目录下创建一个新的 Tauri 应用 `photasa`，将现有的 桌面应用迁移到 Tauri 架构。本 RFC 作为总体架构文档和迁移策略索引，具体的服务迁移细节请参考各个子 RFC。

## 动机

### 为什么选择 Tauri？

1. **性能优势**：Tauri 使用系统原生 WebView，相比 contract reference 的 Chromium 更轻量，启动更快，内存占用更少
2. **安全性**：Rust 的类型安全和内存安全特性，减少潜在的安全漏洞
3. **包体积**：Tauri 应用的最终包体积通常比 桌面应用小 10-50 倍
4. **跨平台一致性**：更好的原生外观和体验
5. **技术栈多样性**：引入 Rust 技术栈，为团队提供更多技术选择

### 当前 桌面应用架构

当前 `legacy-api contract` 采用三层架构：
...
### contract reference Main → Tauri Rust 映射

> **映射表仅索引 reference behavior规格**（IPC 名、事件、磁盘格式）。实现方式为 **Rust 重写**，不得复制 TypeScript 源码或引用 `@photasa/*` Node 包。
...
**contract reference IPC Handler:**

```typescript
// historical main/deity/tianshu-service.ts
...
```typescript
// historical main/import/import-worker.ts
...
**contract reference Preload:**

```typescript
// historical preload/tianshu.ts
export const Tianshu = {
    processCommand: (command) => legacyPreloadAPI.ipcRenderer.invoke("tianshu.command", command),
};
```
...
在 `apps/photasa/src` 中创建适配层，将 legacy preload API 调用转换为 Tauri API：
...
if (isTauri) {
    ...
    // 使用 legacy preload API
    const { ipcRenderer } = require("legacy");
}
```

**推荐使用方案一**，因为它提供了更好的类型安全和代码组织。

### contract reference 端 API/IPC 对照（代码审查）

以下为对 `historical main` 与 `preload/legacy.ts` 的审查结果，迁移时须与此一致。
...
1. **扫描服务 (ScanService)** - [RFC 0068](./completed/0068-tauri-scan-service-migration.md)

- IPC: `picasa:scan-photos` (send), `picasa:find-photo` (main→renderer)
- 功能：照片扫描和索引
- 预计时间：2-3 周

2. **缩略图服务 (ThumbnailService)** - [RFC 0069](./completed/0069-tauri-thumbnail-service-migration.md)
...
**解决方案**：分阶段、按格式迁移，详见 [RFC 0069 的 Image processing support plan](./completed/0069-tauri-thumbnail-service-migration.md#image-processing-support-plan-tauri)。要点：
...
**问题**：contract reference 使用单例锁确保只有一个实例。
...
1. **迁移扫描服务** - [RFC 0068](./completed/0068-tauri-scan-service-migration.md)
...
2. **迁移缩略图服务** - [RFC 0069](./completed/0069-tauri-thumbnail-service-migration.md)
...
- [ ] **扫描服务迁移**（参考 [RFC 0068](./completed/0068-tauri-scan-service-migration.md)）
- [ ] 文件遍历实现
- [ ] 扫描策略决策
- [ ] 增量缓存管理
- [ ] Worker 线程迁移
...
- 需要维护两套代码（contract reference + Tauri）
- **缓解**：共享前端代码，仅后端不同
...
- **Backend**：Rust **重写**（非 TS 复制），legacy main 仅作行为规格（[ROADMAP.md](../../../ROADMAP.md)）
- **Frontend**：复用/adapt `desktop` renderer UI，经 adapter 调 Rust
- **优点**：风险可控，可对比测试，UI 交付快
- **缺点**：过渡期内需维护 contract reference + Tauri 两套后端
...
3. **测试策略**：如何确保 Tauri 版本功能与 legacy-api 版本一致
4. **性能基准**：需要建立性能基准测试，对比两个版本

## 成功标准

1. **功能完整性**：Tauri 版本实现所有 contract reference 版本的核心功能
...
- ✅ [RFC 0068: 扫描服务迁移](./completed/0068-tauri-scan-service-migration.md) - 详细计划已制定
- ✅ [RFC 0069: 缩略图服务迁移](./completed/0069-tauri-thumbnail-service-migration.md) - 详细计划已制定
- ✅ [RFC 0070: 导入服务迁移](./0070-tauri-import-service-migration.md) - 详细计划已制定
...
| RFC                                                               | 服务       | 状态        | 预计时间 | 复杂度 |
| ----------------------------------------------------------------- | ---------- | ----------- | -------- | ------ |
| [RFC 0068](./completed/0068-tauri-scan-service-migration.md)      | 扫描服务   | ✅ 计划完成 | 2-3 周   | 中等   |
| [RFC 0069](./completed/0069-tauri-thumbnail-service-migration.md) | 缩略图服务 | ✅ 计划完成 | 2-3 周   | 中等   |
| [RFC 0070](./0070-tauri-import-service-migration.md)              | 导入服务   | ✅ 计划完成 | 2-3 周   | 中等   |
