...
- **RFC编号**: 0019
- **标题**: Auto-Update System - Client Implementation
- **作者**: 李鹏
- **开始日期**: 2025-09-14
- **状态**: ✅ **已完成**
- **完成日期**: 2025-09-14
...
专注于Photasa桌面应用的客户端自动更新系统实现。采用标准的legacy auto-updater方案，通过服务端URL重写代理解决UploadThing CDN兼容性问题。遵循现有Service架构模式，提供安全的preload集成、完整的UI组件，以及与现有系统的无缝集成。服务器端实现将在RFC 0020中详细说明。

**技术架构**：客户端使用标准legacy auto-updater配置文件驱动，服务端负责URL重写和文件代理，确保文件名安全性和下载兼容性。这种架构保持了客户端的简洁性，同时在服务端统一解决技术问题。
...
1. **客户端零修改**：legacy auto-updater使用标准流程，无需自定义下载逻辑
2. **架构简洁**：问题在服务端统一解决，客户端保持简单
3. **向后兼容**：未来可以轻松切换到其他CDN服务
4. **统计友好**：所有下载都经过服务端，便于统计分析
5. **缓存控制**：可以添加CDN缓存策略和访问控制

#### legacy auto-updater 标准用法保持

**保留所有legacy auto-updater优势**：
...
```
┌─────────────────────────────────────────────────────────────────────┐
│ PHOTASA STANDARD AUTO-UPDATER ARCHITECTURE │
├─────────────────────────────────────────────────────────────────────┤
...
│ ┌─────────────────────────────────────────────────────────────────┐ │
│ │ import { legacyPreloadAPI } from "@legacy-toolkit/preload"; │ │
│ │ │ │
│ │ const api = { │ │
│ │ // ... 现有 API ... │ │
│ │ checkForUpdates: () => legacyPreloadAPI.ipcRenderer.invoke(...), │ │
│ │ onUpdateAvailable: (cb) => legacyPreloadAPI.ipcRenderer.on(...), │ │
│ │ removeUpdateListeners: () => legacyPreloadAPI.ipcRenderer.remove │ │
│ │ }; │ │
│ │ │ │
...
在标准legacy auto-updater方案中，UpdateInfo是核心的数据结构，包含了版本检查和下载所需的所有信息。服务端通过URL重写确保客户端接收到安全的文件路径。
...
- 返回：符合legacy auto-updater格式的YAML配置
...
```yaml
# legacy auto-updater 配置 (dev-app-update.yml 开发环境)
provider: generic
url: https://photasa.me/api/updates/releases
updaterCacheDirName: photasa-updater
# 生产环境配置由 legacy packager.yml 中的 publish 配置自动生成
```
...
import { BrowserWindow, app, shell, type IpcMain } from "desktop-shell";
import { autoUpdater, type UpdateInfo } from "legacy auto-updater";
...
/**
 * 标准更新服务
 * 使用legacy auto-updater标准流程，配合服务端URL重写代理
...
export default class UpdateService {
 ...
 private initDesktopUpdater() {
 // 配置legacy auto-updater - 使用配置文件驱动，不调用setFeedURL
 // 开发环境：自动读取 dev-app-update.yml
 // 生产环境：自动读取内嵌的 app-update.yml (由 legacy packager.yml 生成)

 // 配置行为选项
 autoUpdater.autoDownload = false; // 可选：禁用自动下载
 autoUpdater.autoInstallOnAppQuit = false; // 可选：禁用退出时自动安装

 // 监听legacy auto-updater事件
 this.setupDesktopUpdaterEvents();
 }

 private setupDesktopUpdaterEvents() {
 ...
 * 检查应用更新
 */
 checkForUpdates: () => legacyPreloadAPI.ipcRenderer.invoke('update:check'),

 /**
 * 下载应用更新
 */
 downloadUpdate: () => legacyPreloadAPI.ipcRenderer.invoke('update:download'),

 /**
 * 安装应用更新（会重启应用）
 */
 installUpdate: () => legacyPreloadAPI.ipcRenderer.invoke('update:install'),

 /**
 * 跳过指定版本
 */
 skipVersion: (version: string) => legacyPreloadAPI.ipcRenderer.invoke('update:skip-version', version),

 /**
 * 获取更新配置
 */
 getUpdateConfig: () => legacyPreloadAPI.ipcRenderer.invoke('update:get-config'),

 /**
 * 设置更新配置
 */
 setUpdateConfig: (config: any) => legacyPreloadAPI.ipcRenderer.invoke('update:set-config', config),
 ...
 */
 ...
 legacyPreloadAPI.ipcRenderer.on('update:checking', handler);
 return () => legacyPreloadAPI.ipcRenderer.removeListener('update:checking', handler);
 },
 ...
 */
 ...
 legacyPreloadAPI.ipcRenderer.on('update:available', handler);
 return () => legacyPreloadAPI.ipcRenderer.removeListener('update:available', handler);
 },
 ...
 */
 ...
 const handler = () => callback();
 legacyPreloadAPI.ipcRenderer.on('update:not-available', handler);
 return () => legacyPreloadAPI.ipcRenderer.removeListener('update:not-available', handler);
 },
 ...
 */
 ...
 legacyPreloadAPI.ipcRenderer.on('update:download-progress', handler);
 return () => legacyPreloadAPI.ipcRenderer.removeListener('update:download-progress', handler);
 },
 ...
 */
 ...
 const handler = () => callback();
 legacyPreloadAPI.ipcRenderer.on('update:downloaded', handler);
 return () => legacyPreloadAPI.ipcRenderer.removeListener('update:downloaded', handler);
 },
 ...
 */
 ...
 legacyPreloadAPI.ipcRenderer.on('update:error', handler);
 return () => legacyPreloadAPI.ipcRenderer.removeListener('update:error', handler);
 },
 ...
 */
 ...
 legacyPreloadAPI.ipcRenderer.removeAllListeners('update:checking');
 legacyPreloadAPI.ipcRenderer.removeAllListeners('update:available');
 legacyPreloadAPI.ipcRenderer.removeAllListeners('update:not-available');
 legacyPreloadAPI.ipcRenderer.removeAllListeners('update:download-progress');
 legacyPreloadAPI.ipcRenderer.removeAllListeners('update:downloaded');
 legacyPreloadAPI.ipcRenderer.removeAllListeners('update:error');
 },
};
...
import { ipcMain } from 'desktop-shell';
...
### contract reference 端实施计划

#### Phase 1: 主进程基础设施 (Week 1)

1. **安装和配置 legacy auto-updater**

```bash
npm install legacy auto-updater
...
const api = {
     ...
     * 检查应用更新
     */
    checkForUpdates: () => legacyPreloadAPI.ipcRenderer.invoke("photasa:check-for-updates"),

    /**
     * 下载更新
     */
    downloadUpdate: () => legacyPreloadAPI.ipcRenderer.invoke("photasa:download-update"),

    /**
     * 安装更新并重启应用
     */
    installUpdate: () => legacyPreloadAPI.ipcRenderer.invoke("photasa:install-update"),

    /**
     * 获取当前应用版本
     */
    getAppVersion: () => legacyPreloadAPI.ipcRenderer.invoke("photasa:get-app-version"),

    /**
     * 获取更新配置
     */
    getUpdateConfig: () => legacyPreloadAPI.ipcRenderer.invoke("photasa:get-update-config"),

    /**
     * 设置更新配置
     */
    setUpdateConfig: (config: any) =>
        legacyPreloadAPI.ipcRenderer.invoke("photasa:set-update-config", config),

    /**
     * 跳过特定版本更新
     */
    skipVersion: (version: string) =>
        legacyPreloadAPI.ipcRenderer.invoke("photasa:skip-version", version),
    ...
    onUpdateChecking: (callback: () => void) => {
        const handler = () => callback();
        legacyPreloadAPI.ipcRenderer.on("photasa:update-checking", handler);
        return () =>
            legacyPreloadAPI.ipcRenderer.removeListener("photasa:update-checking", handler);
    },
    ...
    onUpdateAvailable: (callback: (info: any) => void) => {
        const handler = (_: any, info: any) => callback(info);
        legacyPreloadAPI.ipcRenderer.on("photasa:update-available", handler);
        return () =>
            legacyPreloadAPI.ipcRenderer.removeListener("photasa:update-available", handler);
    },
    ...
    onUpdateNotAvailable: (callback: () => void) => {
        const handler = () => callback();
        legacyPreloadAPI.ipcRenderer.on("photasa:update-not-available", handler);
        return () =>
            legacyPreloadAPI.ipcRenderer.removeListener("photasa:update-not-available", handler);
    },
    ...
    onDownloadProgress: (callback: (progress: any) => void) => {
        const handler = (_: any, progress: any) => callback(progress);
        legacyPreloadAPI.ipcRenderer.on("photasa:update-download-progress", handler);
        return () =>
            legacyPreloadAPI.ipcRenderer.removeListener(
                "photasa:update-download-progress",
                handler,
            );
    },
    ...
    onUpdateDownloaded: (callback: () => void) => {
        const handler = () => callback();
        legacyPreloadAPI.ipcRenderer.on("photasa:update-downloaded", handler);
        return () =>
            legacyPreloadAPI.ipcRenderer.removeListener("photasa:update-downloaded", handler);
    },
    ...
    onUpdateError: (callback: (error: any) => void) => {
        ...
        legacyPreloadAPI.ipcRenderer.on("photasa:update-error", handler);
        return () => legacyPreloadAPI.ipcRenderer.removeListener("photasa:update-error", handler);
    },
    ...
    onUpdateRetry: (callback: (retryInfo: any) => void) => {
        ...
        legacyPreloadAPI.ipcRenderer.on("photasa:update-retry", handler);
        return () => legacyPreloadAPI.ipcRenderer.removeListener("photasa:update-retry", handler);
    },

    /**
     * 检查网络连接
     */
    checkNetworkConnectivity: () => legacyPreloadAPI.ipcRenderer.invoke("photasa:check-network"),

    /**
     * 检查磁盘空间
     */
    checkDiskSpace: () => legacyPreloadAPI.ipcRenderer.invoke("photasa:check-disk-space"),

    /**
     * 带重试机制的更新检查
     */
    checkForUpdatesWithRetry: (maxRetries?: number, retryDelay?: number) =>
        legacyPreloadAPI.ipcRenderer.invoke(
...
if (process.contextIsolated) {
    try {
        contextBridge.exposeInMainWorld("legacy", legacyPreloadAPI);
        contextBridge.exposeInMainWorld("api", api);
    } catch (error) {
        console.error(error);
    }
} else {
    window.legacy = legacyPreloadAPI;
    window.api = api;
}
```
...
1. **应用包大小增加**：legacy auto-updater 及相关依赖会增加约 2-3MB 应用体积
...
1. **差量更新实现**：legacy auto-updater 是否支持增量更新？如何优化下载大小？
