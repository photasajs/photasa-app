...
- **作者**: 李鹏
- **开始日期**: 2024-05-24
- **状态**: 🔨 **进行中** - Phase 0-2 已完成，Phase 3 暂缓（RFC 0048 架构已稳定），Phase 4 待实施
- **最后更新**: 2026-07-23
- **类型**: 功能
- **相关RFC**:
...
```typescript
// UI只知道这些简单的IPC调用
window.electron.ipcRenderer.invoke('preference.get')
window.electron.ipcRenderer.invoke('preference.update', delta)
window.electron.ipcRenderer.on('preference.changed', callback)
```
...
**长期目标决策（2026-07-23 更新）**：
- **Phase 3 暂缓**：RFC 0048 的尉迟恭编排架构已稳定运行并通过测试，短期内不迁移编排逻辑到天界
- **当前架构接受为正式方案**：人界 Store SSOT + 天界扫描执行，性能可接受
- **Phase 4 仍待实施**：限流、失败恢复、指标上报
- **千里眼引擎包**（`@photasa/qianliyan`）已独立，队列持久化（RFC 0046）已完成
