# RFC 0098: historical main 模块提取为独立 Packages
...
Photasa / Tauri architecture uses Rust-first backend architecture (`crates/` and direct Rust IPC). Extracting Node/TypeScript main packages (`@photasa/*`) is contract reference legacy backlog and not maintained for Photasa. See [ROADMAP.md](../../ROADMAP.md) Golden Rule.

## Out of scope (Photasa / Tauri)

> **This RFC is deferred.** Photasa backend work follows [ROADMAP.md](../../ROADMAP.md): **Rust rewrite, not TypeScript copy.** Do not import `@photasa/*` packages from Tauri or treat Phase 2 extraction as the migration path for scan/import/config/thumbnail.

- **Phase 2+** (service/worker slimming, further `@photasa/*` growth): **Frozen** for Tauri priority. Resume only if legacy maintenance is explicitly extended.
- Tauri gaps (e.g. RFC 0105 scan cache) are implemented **in Rust**, using contract reference behavior as **spec only**.

## Summary

将 `historical main` 中的纯 Node.js 业务逻辑模块提取为 `packages/@photasa/*` 独立 npm 包，供 桌面应用通过 workspace 依赖引用。这些包不面向 Tauri，也不面向浏览器，目的是在 monorepo 内建立清晰的模块边界、独立测试和版本管理。

## Motivation

### 为什么要提取？

`historical main` 目前是一个"大锅炖"——扫描核心、配置存储、导入处理、缩略图生成等业务逻辑，和 contract reference IPC 胶水代码混在同一个应用目录里。现有的 `@photasa/taiyi`、`@photasa/qianliyan`、`@photasa/siming` 等引擎已经证明了这个提取模式的可行性。

提取的核心收益：

1. **独立测试**：每个包有自己的 vitest 配置，测试不需要启动 contract reference，运行更快
2. **清晰边界**：`*-service.ts` 只负责 IPC 注册，业务逻辑在包里
3. **独立版本**：可以单独对某个包做 breaking change，不影响整个 desktop 应用构建
4. **contract reference hygiene**：在 contract reference 退场前整理 main 模块边界；**不是** Photasa 后端实现路径（Tauri 用 Rust 重写，见 [ROADMAP.md](../../ROADMAP.md)）

### 不是为了 Tauri

这些包是 **Node.js-only**，使用 `fs`、`path`、`klaw`、`worker_threads` 等 Node.js API，无法在浏览器或 Tauri WebView 中运行。**Photasa 后端必须在 Rust 中独立重写**（见 [ROADMAP.md](../../ROADMAP.md)），不得从 Tauri 引用这些包。本 RFC 目的纯粹是 **contract reference monorepo 内部的架构整洁**。

## Scope Analysis（范围分析）

### 域模块分类

| 域             | 文件数 | contract reference IPC 依赖                              | 建议                                     |
| -------------- | ------ | -------------------------------------------------------- | ---------------------------------------- |
...
| `tianting/`    | 10     | ✅ 有（ServiceRegistry 依赖 contract reference）         | 保留在 desktop                           |
| `deity/`       | 3      | ✅ 有（TaiyiService/TianshuService 含 IPC）              | 保留在 desktop                           |
| `window/`      | 1      | ✅ 有（BrowserWindow）                                   | 保留                                     |
| `menu/`        | 1      | ✅ 有（contract reference Menu）                         | 保留                                     |
| `update/`      | 2      | ✅ 有（legacy auto-updater）                             | 保留                                     |
| `watch/`       | 2      | ✅ 有（IPC 通知）                                        | service 保留，核心 shunfenger 已是包     |
| `shell/`       | 1      | ✅ 有（contract reference shell）                        | 保留                                     |
...
1. **Package = 纯 Node.js 业务逻辑**：不 import `legacy`、`ipcMain`、`BrowserWindow`
2. **Service（留在 desktop）= 薄 IPC 包装层**：只做 IPC channel 注册和参数转发，< 100 行
3. **Package 可独立测试**：vitest 直接运行，不需要 contract reference 运行时
...
│ └── status/notify-bridge.ts # ← contract reference：notify:status IPC（payload 来自包内纯函数）
├── import/
│ └── import-service.ts # ← 薄层：IPC 注册 + 调用 @photasa/import
│ └── import-worker.ts # ← 留在 desktop（contract reference IPC 入口，无业务逻辑）
├── thumbnail/
...
3. 移动测试：`scan/__tests__/`（排除 scan-worker.test.ts，该测试依赖 contract reference）
4. 更新 `scan-service.ts` 改为 `import { ... } from '@photasa/scan'`
5. `scan-worker.ts` 留在 desktop（contract reference worker_threads fork）
6. 验证同上
...
3. `import-worker.ts` 留在 desktop（contract reference worker_threads fork）
4. 移动测试
5. 验证同上
...
- [ ] 新包中零 `import ... from 'desktop-shell'`
- [ ] 新包有独立 `vitest.config.ts`，测试不依赖 contract reference 运行时
- [ ] 原有测试 100% 迁移并通过（contract reference 依赖的测试留在 desktop）
- [ ] `legacy-api contract` 中对应 `*-service.ts` 行数 < 100 行
...
- 各包零 contract reference 依赖（`grep -r "from 'desktop-shell'" packages/@photasa/scan` 无结果）
- `historical main/` 各域目录只剩一个 `*-service.ts` 薄层
- 所有原有测试通过，覆盖率无下降

## Resolved Decisions

1. **Worker 文件**：`scan-worker.ts` 和 `import-worker.ts` 是 contract reference IPC 入口文件，无业务逻辑，**留在 desktop**，不提取。
...
RFC 0098 的 **Phase 1（包提取）已完成**：4 个目标包 (`@photasa/config-core`, `@photasa/scan`, `@photasa/import`, `@photasa/thumbnail`) 已建立、已注册、已被 `historical main` 引用。
...
| `scan/status/notify-bridge.ts`   |                                                                         12 | contract reference IPC 桥（payload 构造已在 `@photasa/scan`） |        — | ✅   |
...
**结论**：RFC 0098 Phase 1（包提取）完成。**Phase 2 已冻结**（见 [ROADMAP.md](../../ROADMAP.md)：Photasa 用 Rust 重写，不以继续抽 TS 包为迁移路径）。Status 保持 `Partially Implemented`（deferred）。

### 各文件诊断 — 应迁出哪些代码到哪个包

#### 1. `thumbnail/thumbnail-service.ts` (155 → 目标 ~80)

**应留在 desktop**（contract reference 必需）：
...
**判定**：thumbnail-service 无业务逻辑残留，**纯 IPC 薄层**，超标的 55 行全是 worker 生命周期 + 日志桥接（contract reference 必需）。RFC 的 < 100 行硬指标对此文件不现实。建议把 RFC 的硬指标改为 "**零业务逻辑、零文件 IO、零数据转换**"，thumbnail-service 已合规。
...
- `attemptWorkerRestart()` 重启策略（行 119–142）— 这是通用的"worker 重启策略"，非 contract reference 特有，可作为 `@photasa/common` 或 `@photasa/config-core/worker-supervisor.ts` 工具
...
- `dialog`（contract reference 原生对话框）相关 IPC handler
...
**仍应迁出 / 留在 service 的contract reference 胶**：根据剩余 ~710 行仍需审计；历史上估计仍含：
...
> **Status: Frozen for Photasa priority.** Phase 2 does not substitute Tauri Rust work (RFC 0105, 0097, etc.). Resume only for explicit legacy maintenance.
...
- [ ] 新包模块零 `import ... from 'desktop-shell'`：`grep -r "from 'desktop-shell'" packages/@photasa/<pkg>/src` 无结果
- [ ] 新增的纯函数有单元测试，且**不依赖 contract reference 运行时**
- [ ] 原有测试 100% 通过且无 skip
...
- **零 contract reference 依赖**，零文件 IO
...
4. **修改 service** `historical main/scan/scan-service.ts`
...
- `worker: { postMessage, on }`（最小接口，**不依赖 contract reference Worker 类**）
...
5. **修改 service** `historical main/config/config-service.ts`
...
**风险**：中。worker_threads 边界容易出 bug（序列化、context 隔离），需在真实 legacy preload 环境下手动测试。
...
- `historical main/scan/scan-worker.ts` 已改为调用上述纯函数；`@photasa/scan` 入口已导出该模块。
...
**目标**：把 `import-service.ts` 中的 `activeSessions`、`progressCallbacks` 状态机迁入 `@photasa/import/session-manager.ts`，service 只剩 IPC + worker 桥接 + contract reference `dialog` 调用。
...
- 通过回调注入"发送 worker 任务"的能力（不直接依赖 contract reference worker）
...
**风险**：高。状态机迁出可能引入并发 bug，必须在真实 legacy preload 环境下测试取消、并发、错误恢复路径。
...
4. ~~任务 4 / 5~~ — **不继续**，除非明确延长 旧栈维护期
5. **Photasa 扫描/导入缺口** → 在 Rust 实现（如 RFC 0105），contract reference TS 仅作规格

**关闭条件（修订）**：

- Phase 1：`Implemented`（contract reference 包已提取）✅
...
- [ ] 对应 `@photasa/<domain>` 包的 `__tests__/` 测试不依赖 contract reference 运行时

### 当前状态修正

`Status: Partially Implemented` — Phase 1 ✅；Phase 2 **Frozen**（Photasa 优先 Rust 重写，见 [ROADMAP.md](../../ROADMAP.md)）。

完整 Phase 2 Implemented 的判定（**仅当恢复 旧栈维护时**）：上表 Phase 2 任务 1–5 全部完成，且新 Migration Rules 全部满足。
