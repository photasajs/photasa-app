...
Photasa / Tauri architecture uses Rust-first backend architecture (`crates/` and direct Rust IPC). Extracting Node/TypeScript main packages (`@photasa/taiyi`) is contract reference legacy backlog and not maintained as a Photasa target. See [ROADMAP.md](../../ROADMAP.md) Golden Rule.

Extract the Taiyi Engine (currently in `historical engines/taiyi`) and its workflow adapter helper (`historical engines/workflow`) into a dedicated workspace package `@photasa/taiyi`.
...
├── src/
│ ├── core/ # From historical engines/taiyi/core
│ ├── workflow/ # From historical engines/workflow
...
- Move `historical engines/taiyi` contents to `src/`.
- Move `historical engines/workflow` contents to `src/workflow/`.

3. **Update References**:

- Update imports in `historical main/deity/taiyi-service.ts` to consume `@photasa/taiyi`.
- Add `@photasa/taiyi` as a dependency to `legacy-api contract`.
...
- Keep in `legacy-api contract`: Violates the modularization goal.
