...
| [0136](.spec/rfc/0136-tauri-scan-runtime-contract.md)                        | Tauri persisted queue scan pipeline（贞观职责 + 千里眼一层报告）                       | 📋 Draft                                                          |
...
| [0138](.spec/rfc/0138-tauri-photasa-config-crate.md)                         | `photasa-config` crate（folder-level `.photasa.json`，退出 zouwu）                     | 📋 Draft（crate 已落地，adapter 未删，早期阶段）                  |
...
| [0142](.spec/rfc/0142-tauri-zhenguan-config-commands-personification.md)     | 文件夹配置命令贞观之治对齐（魏征接管）                                                 | 📋 Draft                                                          |
| [0143](.spec/rfc/0143-tauri-zhenguan-scanning-personification.md)            | 扫描与队列命令贞观之治对齐（百姓/尉迟恭/袁天罡）                                       | 📋 Draft                                                          |
...
**编号：** **0108–0110 不回填**；**0118–0143** 已登记。
...
**Closed (also one each):** [0120](.spec/rfc/completed/0120-tauri-import-quit-recovery.md) ✅ · [0121](.spec/rfc/completed/0121-tauri-import-settings-prefs.md) ✅ · [0122](.spec/rfc/rejected/0122-tauri-legacy-importphotos-background-ux.md) ❌ · [0126](.spec/rfc/rejected/0126-electron-import-background-ux-parity.md) ❌.
...
- **0141（2026-07-19）：** 接近完成——`crates/photasa-media` 落地，且 `path.rs`/`photasa-import`/`photasa-thumbnail` 三处消费方均已切换依赖（旧扩展名表已删）。`cargo test --workspace` 265 passed，`cargo clippy` 零警告。**RFC 仍 Draft**：Acceptance 第 3 条"三份原始表差异是否已记录明确决定"尚未见文档确认，Testing strategy 要求的对比测试基线未做（crate 先行落地、基线事后补）。
- **0138（2026-07-19）：** 早期——`crates/photasa-config` crate 存在且测试通过，但 `config_adapter.rs` 未删除、仍用旧 `commands/photasa_config.rs` 路径、仍注册在 zouwu `AdapterRegistry`，renderer 调用链未 trace。距 Acceptance 差距大。
- **0139/0140（2026-07-19）：** 排期与迁移模式 RFC，不含代码改动，均 Draft。
