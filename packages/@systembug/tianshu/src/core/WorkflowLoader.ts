...
export class WorkflowLoader {
    ...
    private logger = createLogger({ context: "workflow-loader" });
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            this.logger.info(`[WorkflowLoader] Initialized with ${this.cache.size} workflows`);
        } catch (error) {
            this.logger.error("[WorkflowLoader] Failed to initialize:", error);
            throw error;
        }
    }
    ...
    async loadWorkflow(workflowId: string): Promise<WorkflowDefinition | null> {
        this.logger.info("🌌 加载工作流", { workflowId });
        ...
        if (cached && this.isCacheValid(cached)) {
            this.logger.info("🌌 从缓存加载工作流", { workflowId });
        ...
        if (workflow) {
            this.logger.info("🌌 缓存工作流", { workflowId });
            this.cacheWorkflow(workflowId, workflow);
        }
        this.logger.info("🌌 加载工作流完成", { workflowId });
        return workflow;
    }
    ...
    async cleanup(): Promise<void> {
        ...
        this.logger.info("[WorkflowLoader] Cleaned up");
    }
    ...
    private async scanAndLoadWorkflows(): Promise<void> {
        try {
            ...
            this.logger.error("[WorkflowLoader] Failed to scan workflows:", error);
            throw error;
        }
    }
    ...
    private async loadWorkflowFromFile(workflowId: string): Promise<WorkflowDefinition | null> {
        try {
            ...
            this.logger.error(`[WorkflowLoader] Failed to load workflow ${workflowId}:`, error);
            return null;
        }
    }
    ...
    private validateWorkflowDefinition(data: any): WorkflowDefinition | null {
        try {
            ...
            this.logger.error("[WorkflowLoader] Invalid workflow definition:", error);
            return null;
        }
    }
    ...
    private async setupFileWatchers(): Promise<void> {
        try {
            ...
            for (const filePath of files) {
                ...
                if (workflowId) {
                    const watcher = fs.watch(filePath, async (eventType) => {
                        if (eventType === "change") {
                            this.logger.info(
                                `[WorkflowLoader] Workflow file changed: ${workflowId}`,
                            );
            ...
            this.logger.error("[WorkflowLoader] Failed to setup file watchers:", error);
        }
    }
}
