...
export class QiZouRouter {
     ...
     * @returns 解析后的值
     */
    private resolveTemplateValue(value: string, qizou: Qizou): unknown {
    ...
    private resolveContent(content: Record<string, string>, qizou: Qizou): Record<string, unknown> {
        ...
        for (const [key, value] of Object.entries(content)) {
            // 递归处理数组、对象和字符串
            resolved[key] = this.resolveValueRecursive(value, qizou);
        }
     ...
     * @returns 解析后的值
     */
    private resolveValueRecursive(value: unknown, qizou: Qizou): unknown {
        // 处理数组
        if (Array.isArray(value)) {
            return value.map((item) => this.resolveValueRecursive(item, qizou));
        }
        ...
        if (value !== null && typeof value === "object") {
            ...
            for (const [k, v] of Object.entries(value)) {
                resolved[k] = this.resolveValueRecursive(v, qizou);
            }
            return resolved;
        }
        ...
        if (typeof value === "string") {
            return this.resolveTemplateValue(value, qizou);
        }
