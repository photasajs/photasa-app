...
> **历史文档（Electron）**：下文描述 Electron 架构。当前交付为 Tauri；见 [ROADMAP.md](../../ROADMAP.md)。

## Overview

Photasa is a **Tauri** desktop application (formerly Electron) designed to scan selected folders, generate thumbnails for quick preview, and manage file metadata. The app uses worker processes for asynchronous tasks, Pinia for state management, and Ant Design Vue for the UI.
