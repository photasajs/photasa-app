...
> **历史文档（contract reference）**：下文描述 historical architecture。当前交付为 Tauri；见 [ROADMAP.md](../../ROADMAP.md)。

## Overview

Photasa is a **Tauri** desktop application (formerly contract reference) designed to scan selected folders, generate thumbnails for quick preview, and manage file metadata. The app uses worker processes for asynchronous tasks, Pinia for state management, and Ant Design Vue for the UI.
...
- Sets up legacy main process, initializes services:
...
## 8. Historical desktop shell architecture
...
- [`src/main/index.ts`](src/main/index.ts)
- Main process, sets up contract reference window, IPC, and services.
