...
历史说明: legacy renderer 组件文档；UI 现位于 apps/photasa/src/
适用范围: Vue3 桌面应用国旗组件（原 历史稿）
---
...
CountryFlag 组件用于根据传入的国家代码，动态渲染对应的 SVG 国旗图标，适配 Tauri + Vue3 桌面应用，保证资源打包、路径兼容性和高可维护性。

## 二、技术方案

- 采用静态 import 方式引入所有支持的 SVG 国旗文件，确保 contract reference 打包后资源可用。
...
- 相关配置见 vite.config.ts、tsconfig.web.json、vitest.config.ts。
