...
本RFC定义天枢引擎YAML工作流的完整语法规范，建立标准化的工作流描述语言，为运行时逻辑描述、AI集成和工作流生成提供统一的语法基础。
...
**用途**：调用外部引擎执行业务逻辑，通过TaiyiService路由到目标引擎。
...
interface WorkflowValidator {
  validateSyntax(workflow: string): ValidationResult;
